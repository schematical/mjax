...
class MJaxFormBase{
        	...
        	if(is_null($objClass->strTemplate)){
				...
				if(!file_exists($objClass->strTemplate)){
					if($objClass->strAssetMode == MJaxAssetMode::MOBILE){
						$objClass->strTemplate  = __VIEW_CORE_DIR__ . '/' . $objClass->strAssetMode . '/form.tpl.php';
					}
				}
			}
        }else{
    ...
    public static function LocateTemplate($strFileLoc, $strAssetMode){
    	if(is_null(self::$strTemplateDirOverride)){
    		$strTemplateDir = __VIEW_APPS_DIR__ .'/'. MLCApplicationBase::$strInitedApp . '/' . $strAssetMode;//__VIEW_MAIN_APP_DIR__;
    	}else{
        ...
        $strFileName = str_replace(__CTL_DIR__, $strTemplateDir, $strFileName);
        
        return $strFileName . '.tpl.php';
    }
	public function RenderTemplate($strFileLoc){
		if(is_null(self::$strTemplateDirOverride)){
    		$strTemplateDir = __VIEW_APPS_DIR__ .'/'. MLCApplicationBase::$strInitedApp . '/' . $this->strAssetMode;//__VIEW_MAIN_APP_DIR__;
    	}else{
    		$strTemplateDir = MJaxForm::$strTemplateDirOverride;
    	}
    ...
    protected function Render(){
    	if($this->blnUseHeader){
	        $strHeader = $this->EvaluateTemplate(__VIEW_CORE_DIR__ . '/' . $this->strAssetMode . '/_header.inc.php');
	        echo($strHeader);
    	}
        ...
        if($this->blnUseHeader){
	        $strFooter = $this->EvaluateTemplate(__VIEW_CORE_DIR__ . '/' . $this->strAssetMode . '/_footer.inc.php');
